...
function h(parameters, flags) {
  ...
  Object.entries(help).forEach(([key, value]) => {
    outputLine(key.replaceAll("_", " ").toLowerCase(), "secondary help-title");
...
function cl(parameters, flags) {
  ...
  const targetCategory =
    bookmarks.find(({ name }) => lowerCaseCompare(name, categoryName)) || null;
  ...
  if (targetCategory === null) {
    ...
    outputMultipleLines([
      `${coloredSpan("created new category", "success")}: ${categoryName}`,
  ...
  const newLink =
    targetCategory.links.find(({ name }) => lowerCaseCompare(name, linkName)) ||
    {};
...
function ccn(parameters, flags) {
  ...
  const targetCategory =
    bookmarks.find(({ name }) => lowerCaseCompare(name, currentName)) || null;
...
function sun(parameters, flags) {
  ...
  outputLine(
    `user name ${coloredSpan(userName, "secondary")} changed to ${coloredSpan(
      newUserName,
      "secondary"
    )}`,
    "success"
...
function sb(parameters, flags) {
  ...
  if (flags.includes("-ab")) {
    ...
    if (isEmptyObject(userBackgrounds)) {
      outputLine(`user backgrounds: ${coloredSpan("empty", "secondary")}`);
    } else {
  ...
  if (backgroundName !== "link" && backgroundName && backgroundLink) {
    outputLine(
      `you have to provide ${coloredSpan("name", "secondary")} or ${coloredSpan(
        "'link' url",
        "secondary"
      )}`,
      "error"
...
function sg(parameters, flags) {
  ...
  if (gifName) {
    ...
    outputLine(`gif ${coloredSpan(gifName, "secondary")} applied`, "success");
  }
}
...
function dug(parameters, flags) {
  ...
  const targetGif =
    Object.keys(userGifs).find((gif) => lowerCaseCompare(gif, gifName)) || null;
...
function ta(parameters, flags) {
  ...
  if (flags.includes("-csl")) {
    outputLine(`line state: ${coloredSpan(lineAnimationState, "secondary")}`);
    outputLine(
